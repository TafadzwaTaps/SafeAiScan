...
async function scanRepoAPI(repoUrl) {
  // Backend route: POST /api/scan-repo
  // Access control is enforced server-side — no client gate needed
...
async function getUsage() {
  try {
    ...
    console.warn("[SecretScan] getUsage failed:", err.message);
    return [];
  }
}
...
async function getHistory() {
  ...
  try {
    ...
    // Graceful fallback — return empty array instead of crashing the dashboard
    if (err.message.includes("404") || err.message.includes("not found")) {
      console.warn("[SecretScan] /api/history not found — returning empty array");
      return [];
    }
    throw err;
  }
...
async function getMe() {
  ...
  try {
    ...
    if (!window.DEV_MODE) {
      ...
      // Legacy cachePlanData for any callers that still use it
      if (data?.plan) cachePlanData(data.plan, data.limits || null);
    }
    return data;
  } catch (err) {
    if (err.message.includes("404")) {
      console.warn("[SecretScan] /api/me not found — using cached values");
      // Return what we have cached rather than crashing the dashboard
      return {
...
window.addEventListener("unhandledrejection", e => {
  if (e.reason instanceof PlanError) {
    ...
    e.preventDefault();
  } else if (e.reason && e.reason.message && e.reason.message.includes("404")) {
    // Suppress 404 noise — each caller handles gracefully
    console.warn("[SecretScan] Suppressed 404:", e.reason.message);
    e.preventDefault();
  } else {
