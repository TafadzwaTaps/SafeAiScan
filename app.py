...
# ============================================================
# AUTH DEPENDENCY — resolves JWT -> user -> org
# Uses request-scoped cache so org is never fetched twice.
# ============================================================
def get_user(
    request: Request,
    ...
    # Fetch org once — DB layer has its own short-TTL cache
    org = None
    if user.get("org_id"):
...
def track_usage(user_id: str, org_id: str) -> int:
    ...
    try:
        ...
        return 1  # never block a scan over tracking failure
    ...
    for pattern, severity, description in _PATTERNS:
        if pattern.lower() in lower and pattern not in seen:
            ...
            findings.append({
                ...
                "fix":         "Review this pattern and sanitize inputs. See OWASP for secure alternatives.",
                "cve":         "N/A",
                "cvss":        5.0,
            })
    return findings
...
async def ai_enrich(text: str, findings: list, depth: str = "full") -> dict:
    ...
    if depth == "basic":
        if findings:
            return {
                "explanation": (
                    f"Detected {len(findings)} potential security issue(s). "
                    "Upgrade to Pro for detailed AI analysis and step-by-step remediation."
                ),
                ...
                res = await client.post(
                    ...
                    json={
                        ...
                        "messages": [{
                            "role": "user",
                            "content": (
                                "You are a senior cybersecurity engineer. Analyze the code below.\n\n"
                                f"Known issues: {findings}\n\n"
...
def register(req: RegisterRequest, request: Request):
    try:
        ...
        logger.info(f"Registering {req.email}")
...
def rotate_api_key(auth=Depends(get_user)):
    ...
    try:
        ...
        logger.info(f"API key rotated: {user['id']}")
...
# ============================================================
# ROUTES — DASHBOARD (aggregated, minimal round-trips)
# ============================================================

@app.get("/api/dashboard")
def get_dashboard(auth=Depends(get_user)):
    """
    Single endpoint for the dashboard — replaces 4 separate calls.
    Makes 2–3 Supabase round-trips total.
    """
    user   = auth["user"]
    org    = auth.get("org")
    plan   = user.get("plan", "free").lower()
...
# ============================================================
# ROUTES — USAGE + HISTORY (kept for backward compat)
# ============================================================

@app.get("/api/usage")
def get_usage(auth=Depends(get_user)):
    user = auth["user"]
    return ok(DB.fetch_usage_history(user["id"], limit=30))
...
async def analyze(req: AnalyzeRequest, request: Request, auth=Depends(get_user)):
    try:
        ...
        risk      = ("CRITICAL" if hi_count >= 3 else
                     "HIGH"     if hi_count >= 1 else
        ...
        DB.write_audit_log(
            user["id"], "scan", org_id=org_id,
            resource="/api/analyze", ip_address=request.client.host,
...
def generate_pdf(data: dict, auth=Depends(get_user)):
    ...
    try:
        ...
        content = [
            Paragraph("SafeAIScan Security Report", styles["Title"]),
            Spacer(1, 12),
            Paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M UTC')}", styles["Normal"]),
            Paragraph(f"Plan: {plan.upper()}", styles["Normal"]),
            Spacer(1, 20),
        ]
        if findings:
    ...
    try:
        ...
        def build_tree(contents, depth=0):
            ...
            for f in contents[:50]:
                ...
                if f.type == "dir":
                    try:
                        node["children"] = build_tree(repo.get_contents(f.path), depth + 1)
                    except Exception:
                        node["children"] = []
                tree.append(node)
            return tree
